...
class EmergenciasMuniCbaSupabaseUser extends BaseAuthUser {
  EmergenciasMuniCbaSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> emergenciasMuniCbaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EmergenciasMuniCbaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
